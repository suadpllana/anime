    ...
    <div className="filtered-anime-container">
      <div className="important-info">
        ...
        <div>
          ...
          <p>{anime?.type} <GoDotFill /> {anime?.duration} <GoDotFill /> {anime?.score}⭐ <GoDotFill /> {anime?.status} <br /> <br />
            {anime?.aired?.string} <GoDotFill />  {anime?.genres?.map((genre) => (<span key={genre?.name}>{genre?.name}</span>))}</p>
      ...
      <div className="side-info">
       ...
       <h2>Recommeded from {anime?.title_english}</h2>
       <div className="recommended-anime">
       {recommendedAnimes?.map(anime => (
        <div key={anime.entry.mal_id}>
          <img src={anime?.entry?.images?.jpg?.image_url} alt="" />
        </div>
       ))}
       </div>
